...
const About = () => {
  ...
  // Arte ASCII do Stitch escolhida por ti
  const fullAsciiArt = `
    ⠀⠀⠀⠀⢀⠃⠀⠀⠀⠀⠀⡼⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⢸⠀⠀⠀⠀⠀⢰⢳⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⡄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⢀⡄⠀⠸⡀⠀⠀⠀⠀⡎⠈⡄⠀⠀⢀⣤⣤⡒⠶⠒⠒⠒⠚⠟⢷⣶⣤⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣠⠒⠉⠀
...
⠀⠀⠀⠀⠀⠀⠀⠀⡠⠊⠘⡄⢳⡀⠀⠀⠀⠀⠀⠀⠀⠀⢠⠇⠀⠈⠉⢖⡒⠒⠉⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
  ...
  // 1. SUBSTITUIÇÃO DOS AUTOCOLANTES POR SÍMBOLOS DE CÓDIGO
  const techSymbols = ['</>', '{ }', 'JS', 'TS', '[ ]', '#', '()', 'CSS', 'HTML', '/>', 'React', 'C#', 'SQL', 'UX', '3D', 'Web'];

  const stickers = [
    { id: 1, initialX: -180, initialY: -80, finalX: -550, finalY: -100, mobileInitialX: -120, mobileInitialY: -60, mobileFinalX: -250, mobileFinalY: -80 },
    { id: 2, initialX: 180, initialY: -60, finalX: 600, finalY: -250, mobileInitialX: 120, mobileInitialY: -40, mobileFinalX: 200, mobileFinalY: -120 },
    { id: 3, initialX: -160, initialY: 240, finalX: -200, finalY: 380, mobileInitialX: -100, mobileInitialY: 160, mobileFinalX: -120, mobileFinalY: 220 },
    { id: 4, initialX: 190, initialY: 260, finalX: 500, finalY: 150, mobileInitialX: 110, mobileInitialY: 180, mobileFinalX: 180, mobileFinalY: 120 },
    { id: 5, initialX: -200, initialY: 120, finalX: -200, finalY: -380, mobileInitialX: -130, mobileInitialY: 80, mobileFinalX: -130, mobileFinalY: -180 },
    { id: 6, initialX: 170, initialY: 100, finalX: 150, finalY: -360, mobileInitialX: 110, mobileInitialY: 70, mobileFinalX: 100, mobileFinalY: -160 },
    { id: 7, initialX: -130, initialY: -130, finalX: -450, finalY: -380, mobileInitialX: -90, mobileInitialY: -90, mobileFinalX: -200, mobileFinalY: -200 },
    { id: 8, initialX: 150, initialY: 200, finalX: 200, finalY: 350, mobileInitialX: 100, mobileInitialY: 140, mobileFinalX: 130, mobileFinalY: 200 },
    { id: 9, initialX: -140, initialY: 300, finalX: -500, finalY: 200, mobileInitialX: -90, mobileInitialY: 200, mobileFinalX: -180, mobileFinalY: 160 },
    { id: 10, initialX: 200, initialY: 120, finalX: 500, finalY: -380, mobileInitialX: 130, mobileInitialY: 80, mobileFinalX: 200, mobileFinalY: -180 },
    { id: 11, initialX: -220, initialY: -40, finalX: 600, finalY: 10, mobileInitialX: -140, mobileInitialY: -30, mobileFinalX: 220, mobileFinalY: 10 },
    { id: 12, initialX: 110, initialY: -180, finalX: 500, finalY: 300, mobileInitialX: 80, mobileInitialY: -120, mobileFinalX: 180, mobileFinalY: 180 },
    { id: 13, initialX: -120, initialY: 360, finalX: 500, finalY: -100, mobileInitialX: -80, mobileInitialY: 240, mobileFinalX: 180, mobileFinalY: -80 },
    { id: 14, initialX: 210, initialY: 40, finalX: -640, finalY: -220, mobileInitialX: 140, mobileInitialY: 30, mobileFinalX: -220, mobileFinalY: -140 },
    { id: 15, initialX: -100, initialY: 160, finalX: -400, finalY: 320, mobileInitialX: -70, mobileInitialY: 110, mobileFinalX: -150, mobileFinalY: 200 },
    { id: 16, initialX: 130, initialY: -100, finalX: -600, finalY: 100, mobileInitialX: 90, mobileInitialY: -70, mobileFinalX: -200, mobileFinalY: 80 },
  ];
  ...
  const getStickerStyle = (sticker: typeof stickers[0]) => {
    ...
    const scale = isVerySmall ? 0.6 + (0.2 * progress) : isMobile ? 0.8 + (0.3 * progress) : 1 + (0.5 * progress);
    const opacity = Math.max(0, 0.7 - (0.3 * progress));
          ...
          <div className="flex flex-col md:flex-row justify-between items-start max-w-6xl mx-auto gap-8">
            <div className="text-left w-full md:w-auto">
              <div className="ascii-container justify-start text-3xl md:text-4xl lg:text-5xl">
                <AsciiMorphText text="Hi, I'm Maria" />
              </div>
      ...
      <div className="py-8 md:py-12" style={{
        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="flex items-center justify-center relative min-h-[400px] md:min-h-[600px]">
            ...
            {/* 1. AUTOCOLANTES SUBSTITUÍDOS POR CÓDIGO FLUTUANTE */}
            <div className="absolute inset-0 flex items-center justify-center overflow-hidden">
              {stickers.map((sticker, index) => (
                <div
                  key={sticker.id}
                  className="absolute z-10 pointer-events-none select-none font-mono font-bold text-2xl md:text-4xl"
            ...
            {/* 2. CADERNO SUBSTITUÍDO POR CARTÃO DIGITAL (ELEVATOR PITCH) */}
            <div
              className="w-full md:max-w-3xl lg:max-w-4xl relative z-20 p-6 md:p-8 flex flex-col md:flex-row gap-6 items-center rounded-3xl shadow-lg backdrop-blur-sm transition-all duration-300"
              style={{
                backgroundColor: isDarkMode ? 'rgba(31, 41, 55, 0.4)' : 'rgba(255, 255, 255, 0.5)',
                border: `1px solid ${withAlpha(themeColors.primary, 0.3)}`
              }}
            >
              {/* Foto Interativa */}
              <div className="w-full md:w-1/3 flex justify-center">
                <img
                  ...
                  alt="Maria Rocha"
                  className="w-48 h-48 md:w-64 md:h-64 object-cover rounded-full shadow-lg cursor-pointer hover:scale-105 transition-transform duration-300"
                  onClick={() => setShowProfileModal(true)}
                  style={{ border: `4px solid ${themeColors.colors.pink[400]}` }}
                />
              </div>

              {/* Texto do Pitch */}
              <div className="w-full md:w-2/3 text-base md:text-lg leading-relaxed" style={{ color: isDarkMode ? '#E5E7EB' : '#374151' }}>
                <h2 className="text-3xl font-bold mb-4" style={{ color: themeColors.primary }}>
                  About Me
                </h2>
                <div className="space-y-4">
                  <p>
                    Hi! I'm Maria, and I'm currently finishing my Bachelor's degree in Software Engineering at ISEP. Throughout my studies, I discovered a strong passion for <strong>Full Stack</strong> development, with a special focus on the <strong>Frontend</strong>, where I can bring ideas to life and focus on the user experience.
                  </p>
                  <p>
                    I also really enjoyed exploring 3D environments and game development. I'm a strong team player who loves sharing knowledge and solving problems collaboratively. I'm highly persistent: if you give me a challenge, I will see it through to the end.
                  </p>
                  <p>
                    Lately, I've been thinking a lot about my future, and I know I want to use technology to help others in their daily lives. I would love to work on developing healthcare applications or, eventually, explore robotics to create solutions that make a real difference.
                  </p>
                </div>
              </div>
            </div>
